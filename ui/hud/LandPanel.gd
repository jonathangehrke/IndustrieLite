...
# Preload icons to ensure they're included in export builds
const ICON_BUY_LAND := preload("res://assets/tools/landkaufen.png")
const ICON_SELL_LAND := preload("res://assets/tools/landverkaufen.png")
...
func _build_ui():
	...
	# Icon aus BuildingDef oder DataIndex beziehen
	...
	if _ensure_ui_service() and ui_service.has_method("GetBuildingDef"):
		...
		if def:
			...
			if "Icon" in def and def.Icon:
				icon_to_use = def.Icon
	var di := get_node_or_null("/root/DataIndex")
	if icon_to_use == null and di and di.has_method("get_building_icon"):
		icon_to_use = di.get_building_icon("road")
	if icon_to_use:
		var tb := TextureButton.new()
		tb.texture_normal = icon_to_use
		...
		var iw = int(icon_to_use.get_width())
		var ih = int(icon_to_use.get_height())
	...
	var buy_tex: Texture2D = ICON_BUY_LAND
	if buy_tex != null:
	...
	var sell_tex: Texture2D = ICON_SELL_LAND
	if sell_tex != null:
