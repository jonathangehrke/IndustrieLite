...
func _get_product_icon(product_id: String) -> Texture2D:
	...
	if tex == null:
		# Primär: DataIndex verwenden (preloaded für Export)
		var data_index = get_node_or_null("/root/DataIndex")
		if data_index and data_index.has_method("get_resource_icon"):
			tex = data_index.get_resource_icon(product_id)

		# Fallback: Runtime-Loading (nur wenn DataIndex nicht verfügbar)
		if tex == null:
			match product_id:
				"chickens":
...
func refresh():
	...
	# Fallback: Hole Orders direkt aus ManagerCoordinator/TransportManager, falls UIService leer ist
	var sc := _get_service_container()
	if sc:
		var coord = sc.GetNamedService("ManagerCoordinator")
		if coord and coord.has_method("GetOrders"):
			var orders2 = coord.GetOrders()
			if typeof(orders2) == TYPE_ARRAY and orders2.size() > 0:
				dbg_ui("MarketPanel: refresh fallback orders count= " + str(orders2.size()))
				set_orders(orders2)
...
func _is_product_unlocked(product_id: String) -> bool:
	# Primär: UIService-Filter (nutzt Database + LevelManager und ist stabiler im Export)
	if _ensure_ui_service() and ui_service.has_method("GetResourcesById"):
		var resources_map: Dictionary = ui_service.GetResourcesById()
		if resources_map != null:
			return resources_map.has(product_id)

	# Sekundär: MarketService (nur wenn vorhanden und vollständig initialisiert)
	var market_service = _get_market_service()
	if market_service and market_service.has_method("IsProductUnlocked"):
		var ok = market_service.call("IsProductUnlocked", product_id)
		# Falls Service (noch) nicht bereit ist, null/false kann bedeuten: später erneut prüfen
		if typeof(ok) == TYPE_BOOL:
			return ok

	# Fallback: DataIndex + LevelManager direkt
	var level := 1
	var sc := _get_service_container()
	if sc:
	...
	if di and di.has_method("get_resources"):
		var res = di.get_resources()
		for r in res:
			if r != null and "Id" in r and str(r.Id) == product_id:
				var req := 1
				if "RequiredLevel" in r:
					req = int(r.RequiredLevel)
				return req <= level

	# Letzter Fallback: sichtbar lassen
	return true
