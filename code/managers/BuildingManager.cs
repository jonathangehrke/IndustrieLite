...
public partial class BuildingManager : Node, ILifecycleScope
{
    ...
    /// <summary>
    /// Alle registrierten Geb4ude in der Szene.
    /// </summary>
    public List<Building> Buildings { get; private set; } = new List<Building>();
    /// <summary>
    /// Alle registrierten St4dte in der Szene.
    /// </summary>
    ...
    public bool CanPlace(string type, Vector2I cell, out Vector2I size, out int cost)
    {
        ...
        try
        {
            var ok = placementService.CanPlace(type, cell, Buildings, out size, out cost);
        ...
        catch (Exception ex)
        {
            DebugLogger.Error("debug_building", "CanPlaceException", ex.Message,
                new System.Collections.Generic.Dictionary<string, object?> { { "type", type }, { "cell", cell } });
            return false;
        }
    }
    ...
    public Building? PlaceBuilding(string type, Vector2I cell)
    {
        ...
        try
        {
            ...
            var b = buildingFactory.Create(type, cell, TileSize);
            if (b == null)
            {
    ...
    public Result<bool> CanPlaceEx(string type, Vector2I cell)
    {
        try
        {
            if (string.IsNullOrWhiteSpace(type) || placementService == null)
            {
            ...
            if (!placementService.CanPlace(type, cell, Buildings, out var size, out var cost))
            {
    ...
    public Result<Building> TryPlaceBuilding(string type, Vector2I cell, string? correlationId = null)
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(type))
                return Result<Building>.Fail(new ErrorInfo(ErrorIds.ArgumentNullName, "Building-Typ fehlt", new Dictionary<string, object?> { { "param", nameof(type) } }));

            if (placementService == null || buildingFactory == null)
            ...
            DebugLogger.Info("debug_building", "PlaceBuildingRequested", $"Platzierung angefragt: {type} @ {cell}",
                new Dictionary<string, object?> { { "type", type }, { "cell", cell } }, correlationId);

            var res = placementService.TryPlace(type, cell, TileSize, Buildings, buildingFactory);
            if (!res.Ok)
            {
                ...
                DebugLogger.Warn("debug_building", "PlaceBuildingFailed", msg,
                    new Dictionary<string, object?> { { "type", type }, { "cell", cell }, { "code", code } }, correlationId);
                return Result<Building>.Fail(res.ErrorInfo ?? new ErrorInfo(ErrorIds.BuildingInvalidPlacementName, msg,
                    new Dictionary<string, object?> { { "type", type }, { "cell", cell } }));
            }
            ...
            DebugLogger.Info("debug_building", "PlaceBuildingSucceeded", $"Platzierung ok: {type} @ {cell}",
                new Dictionary<string, object?> { { "type", type }, { "cell", cell }, { "id", b.BuildingId } }, correlationId);
    ...
    private void AddPlacedBuilding(Building b, Vector2I cell)
    {
        ...
        if (b is ChickenFarm) b.AddToGroup("chicken_farms");

        if (b is City city)
        {
            Cities.Add(city);
            DebugLogger.LogBuilding(() => $"BuildingManager: City '{city.CityName}' placed at {cell}, total cities: {Cities.Count}");
        }
        ...
        if (SignaleAktiv && eventHub != null)
        {
            try
            {
                eventHub.EmitSignal(EventHub.SignalName.BuildingPlaced, b);
                if (b is ChickenFarm)
                {
                    eventHub.EmitSignal(EventHub.SignalName.FarmStatusChanged);
                }
            }
            catch (Exception ex)
            {
    ...
    {
        return Buildings.OfType<ChickenFarm>().ToList();
    }
    ...
    public Godot.Collections.Array<ChickenFarm> GetChickenFarmsForUI()
    {
        ...
        foreach (var farm in Buildings.OfType<ChickenFarm>())
            arr.Add(farm);
        return arr;
    }
    ...
    {
        return Buildings.OfType<SolarPlant>().ToList();
    }

    public List<WaterPump> GetWaterPumps()
    {
        return Buildings.OfType<WaterPump>().ToList();
    }

    public List<House> GetHouses()
    {
        return Buildings.OfType<House>().ToList();
    }

    public bool RemoveBuildingAt(Vector2I cell)
    {
        var b = GetBuildingAt(cell);
        if (b == null) return false;
        return RemoveBuilding(b);
    }
    ...
    public Result TryRemoveBuilding(Building b, string? correlationId = null)
    {
        ...
        try
        {
            UnregisterBuildingGuid(b);
            Buildings.Remove(b);
            if (b is City city) Cities.Remove(city);
            registry?.Remove(b);

            if (SignaleAktiv && eventHub != null)
            {
                try { eventHub.EmitSignal(EventHub.SignalName.BuildingDestroyed, b); }
                catch (Exception ex)
                {
                    DebugLogger.Error("debug_building", "RemoveBuildingSignalError", ex.Message,
                        new System.Collections.Generic.Dictionary<string, object?> { { "building", b.Name } }, correlationId);
                }
            }

            if (IsInstanceValid(b)) b.QueueFree();
            DebugLogger.Info("debug_building", "RemoveBuildingSucceeded", $"Gebaeude entfernt: {b.Name}",
                new System.Collections.Generic.Dictionary<string, object?> { { "pos", b.GridPos }, { "type", b.GetType().Name } }, correlationId);
        ...
        catch (Exception ex)
        {
            ...
            return Result.FromException(ex, ErrorIds.SystemUnexpectedExceptionName, "Unerwarteter Fehler beim Entfernen des Gebaeudes",
                new System.Collections.Generic.Dictionary<string, object?> { { "building", b?.Name } });
        }
    }
    ...
    public Building? GetBuildingByGuid(Guid id)
    {
        ...
        try
        {
            if (buildingsByGuid.TryGetValue(id, out var building))
            {
                if (building != null && IsInstanceValid(building))
                    return building;

                // Clean up invalid reference
                buildingsByGuid.Remove(id);
            }
            return null;
        }
        catch (Exception ex)
        {
            DebugLogger.Error("debug_building", "GetBuildingByGuidException", ex.Message,
                new System.Collections.Generic.Dictionary<string, object?> { { "guid", id } });
            return null;
        }
    }
        ...
        try
        {
            ...
            foreach (var building in Buildings)
            {
                ...
                if (building is IHasInventory inventoryBuilding)
                {
                    ...
                    catch (Exception ex)
                    {
                        DebugLogger.Error("debug_building", "GetInventoryException", ex.Message,
                            new System.Collections.Generic.Dictionary<string, object?> { { "building", building.Name } });
                    }
                }
                ...
                if (!countedFromInventory && building is IHasStock stockBuilding)
                {
                    try
                    {
                        StringName mainResourceId = GetMainResourceIdForBuilding(building);
