...
public partial class City : Building, ITickable
{
    ...
    private bool IsProductUnlockedForGeneration(string displayName)
    {
        try
        {
            ...
            // 1) ID normalisieren (MarketService oder lokale Heuristik)
            string id = displayName.ToLowerInvariant();
            var marketService = sc?.GetNamedService<MarketService>(ServiceNames.MarketService);
            if (marketService != null)
            {
                id = marketService.NormalizeProductName(displayName);
            }
            else
            {
                // Einfache Heuristik
                if (id.Contains("getreide") || id.Contains("grain") || id.Contains("korn")) id = ResourceIds.Grain;
                else if (id.Contains("huhn") || id.Contains("huehner") || id.Contains("chicken")) id = ResourceIds.Chickens;
                else if (id.Contains("schwein") || id.Contains("pig")) id = ResourceIds.Pig;
                else if (id.Contains("ei") || id.Contains("egg")) id = ResourceIds.Egg;
            }

            // 2) Primär: UIService (nutzt Database + Level)
            var uiService = sc?.GetNamedService<UIService>(ServiceNames.UIService);
            if (uiService != null)
            {
                try
                {
                    var map = uiService.GetResourcesById();
            ...
            // 3) Sekundär: MarketService (falls vorhanden)
            if (marketService != null)
            {
                var unlocked = marketService.IsProductUnlocked(id);
                // Wenn Service bereit -> Ergebnis nutzen
                if (unlocked)
                {
                    return true;
                }
                // Wenn false, später noch DataIndex/Level prüfen, um initiale Leere zu vermeiden
            }

            // 4) Fallback: DataIndex + LevelManager direkt
            int level = 1;
            var lm = sc?.GetNamedService<LevelManager>(nameof(LevelManager));
            if (lm != null)
            {
            ...
            if (di != null)
            {
                try
                {
                    var resVar = di.Call("get_resources");
                    if (resVar.VariantType != Variant.Type.Nil)
                    {
                        foreach (var v in (Godot.Collections.Array)resVar)
                        {
                            var res = v.AsGodotObject();
                            if (res is GameResourceDef def && !string.IsNullOrEmpty(def.Id) && def.Id == id)
                            {
            ...
            // 5) Letzter Fallback: sichtbar lassen, um Order-Generierung nicht zu blockieren
            return true;
        }
        catch
        {
