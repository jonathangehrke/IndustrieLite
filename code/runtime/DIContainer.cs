...
public partial class DIContainer : Node
{
	...
	private void InitializeAll()
	{
		...
		RegisterForUI(sc, ServiceNames.GameManager, gameManager);
		RegisterForUI(sc, nameof(LandManager), landManager);
		RegisterForUI(sc, nameof(EconomyManager), economyManager);
		RegisterForUI(sc, nameof(BuildingManager), buildingManager);
		RegisterForUI(sc, nameof(TransportManager), transportManager);
		RegisterForUI(sc, nameof(RoadManager), roadManager);
		RegisterForUI(sc, nameof(InputManager), inputManager);
		RegisterForUI(sc, nameof(ResourceManager), resourceManager);
		RegisterForUI(sc, nameof(ProductionManager), productionManager);
		RegisterForUI(sc, ServiceNames.GameClockManager, gameClockManager);
		RegisterForUI(sc, nameof(CityGrowthManager), cityGrowthManager);
		RegisterForUI(sc, nameof(GameTimeManager), gameTimeManager);
		RegisterForUI(sc, ServiceNames.Simulation, simulation);
		RegisterForUI(sc, nameof(ManagerCoordinator), managerCoordinator);
		RegisterForUI(sc, nameof(SaveLoadService), saveLoadService);
		RegisterForUI(sc, ServiceNames.ResourceRegistry, resourceRegistry);

		// Helper Services (Phase 6)
		RegisterForUI(sc, nameof(LogisticsService), logisticsService);
		RegisterForUI(sc, nameof(MarketService), marketService);
		RegisterForUI(sc, nameof(SupplierService), supplierService);
		RegisterForUI(sc, nameof(ProductionCalculationService), productionCalculationService);
		RegisterForUI(sc, nameof(ResourceTotalsService), resourceTotalsService);

		// ========== Phase 5: Validierung (alle Manager initialisiert?) ==========
		ValidateComposition(landManager, economyManager, buildingManager, transportManager, roadManager, inputManager, resourceManager, productionManager, gameClockManager, cityGrowthManager, simulation, uiService);
		...
		UIService? uiService)
	{
		...
		if (landManager == null) errors.Add("LandManager");
		if (economyManager == null) errors.Add("EconomyManager");
		if (buildingManager == null) errors.Add("BuildingManager");
		if (resourceManager == null) errors.Add("ResourceManager");
		if (productionManager == null) errors.Add("ProductionManager");
		if (simulation == null) errors.Add("Simulation");

		// Optionale Manager (Warning statt Error)
		if (transportManager == null) GD.PushWarning("DIContainer: TransportManager nicht gefunden (optional)");
		if (roadManager == null) GD.PushWarning("DIContainer: RoadManager nicht gefunden (optional)");
		if (inputManager == null) GD.PushWarning("DIContainer: InputManager nicht gefunden (optional)");
		if (gameClockManager == null) GD.PushWarning("DIContainer: GameClockManager nicht gefunden (optional)");
		if (cityGrowthManager == null) GD.PushWarning("DIContainer: CityGrowthManager nicht gefunden (optional)");
		if (uiService == null) GD.PushWarning("DIContainer: UIService nicht gefunden (optional)");
