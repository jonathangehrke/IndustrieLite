...
public partial class Map : Node2D, ILifecycleScope
{
	...
	[Export] public string GrasTexturPfad { get; set; } = "res://assets/tiles/gras.png"; // Pfad zur Gras-Textur
	[Export] public double KaufFeedbackDauer { get; set; } = 1.0; // Dauer des Kauf-Feedbacks in Sekunden
	[Export] public double MaxRedrawHz { get; set; } = 60.0; // Begrenzung der Redraw-Rate
	...
	public override void _Ready()
	{
		...
		EnsureViewportConnected();

		try
		{
			grasTex = ResourceLoader.Load<Texture2D>(GrasTexturPfad);
		}
		catch
		{
			grasTex = null;
		...
		CallDeferred(nameof(EnsureViewportConnected));

		visualFeedback = new MapVisualFeedbackSystem();
		visualFeedback.Name = "VisualFeedback";
		visualFeedback.AnzeigeDauer = KaufFeedbackDauer;
		visualFeedback.DebugAusgabe = DebugLogs;
		AddChild(visualFeedback);

		// Self-registration in ServiceContainer (Named only)
		try { ServiceContainer.Instance?.RegisterNamedService(nameof(Map), this); } catch { }
	}
	...
	public void TriggerPurchaseFeedback(Vector2I cell)
	{
		...
		{
			visualFeedback.AktiviereFeedback(cell);
		}
		else
		{
			RequestRedraw();
		}
		DebugLogger.LogInput("Purchase feedback triggered for cell: " + cell, DebugLogs);
	}
	...
	public Vector2I BerechneCellVonPosition(Vector2 weltPosition)
	{
		...
		int tileGroesse = game.BuildingManager.TileSize;
	...
	{
		sinceLastRedraw += delta;
		debugCounterTimer += delta;

		if (forceRedraw)
		{
			double minInterval = MaxRedrawHz > 0 ? (1.0 / MaxRedrawHz) : 0.0;
			if (sinceLastRedraw >= minInterval)
			{
				QueueRedraw();
				redrawCounter++;
				sinceLastRedraw = 0.0;
	...
	public override void _Draw()
	{
		...
		var koordinator = game.ManagerCoordinator;
		if (koordinator == null) return;

		if (debugCounterTimer >= 1.0)
		{
			DebugLogger.LogPerf(() => $"Map: Redraws/s ~ {redrawCounter}");
			redrawCounter = 0;
			debugCounterTimer = 0.0;
		}

		int w = game.LandManager.GridW;
		int h = game.LandManager.GridH;
		int ts = game.BuildingManager.TileSize;
		...
		Vector2I? feedbackCell = visualFeedback?.AktuelleZelle;
		...
		for (int x = minX; x <= maxX; x++)
		{
			for (int y = minY; y <= maxY; y++)
			{
				...
				if (koordinator.IsOwned(cell))
				{
					...
					{
						DrawRect(rect, purchaseFeedbackColor);
					}
					else
					{
						if (grasTex != null)
						{
							DrawTextureRect(grasTex, rect, false);
						}
						else
						{
							DrawRect(rect, landColor);
						}
					}

					if (koordinator.IsSellLandModeActive() && koordinator.CanSellLand(cell))
					{
						DrawRect(rect, sellableColor);
					}
				}
				else if (koordinator.IsBuyLandModeActive() && koordinator.CanBuyLand(cell))
				{
					DrawRect(rect, buyableColor);
				}
