...
{
    private static readonly string TEST_SAVE_DIR = "user://test_saves/";
    ...
    {
        if (state1.OwnedCells.Count != state2.OwnedCells.Count) return false;
        if (state1.Buildings.Count != state2.Buildings.Count) return false;

        // Detaillierte Vergleiche
        foreach (var cell in state1.OwnedCells)
        {
            if (!state2.OwnedCells.Contains(cell)) return false;
        }
        ...
        for (int i = 0; i < state1.Buildings.Count; i++)
        {
            ...
            if (b1.Stock != b2.Stock) return false;

            if (b1.Inventory.Count != b2.Inventory.Count) return false;
            ...
            case JsonValueKind.Object:
                {
                    ...
                    {
                        if (!propsB.TryGetValue(kv.Key, out var bv)) return false;
            ...
            case JsonValueKind.Array:
                {
                    ...
                    using (var ib = b.EnumerateArray().GetEnumerator())
                    {
                        ...
                        {
                            if (!JsonDeepEquals(ia.Current, ib.Current)) return false;
                        }
                    }
                    return true;
                }
    ...
    {
        var testDir = ProjectSettings.GlobalizePath(TEST_SAVE_DIR);
...
public class SaveLoadTestResult
{
    ...
    public override string ToString()
    {
        if (TestSuccessful)
            ...
            return $"Round-trip test FAILED - {ErrorMessage}";
    }
}
