...
public static class DebugLogger
{
    ...
    private static Node? _devFlags;
    private static bool _triedResolve = false;

    // Globale Mindest-Logstufe; kann zur Laufzeit via API angepasst werden
    private static LogLevel _minLevel = OS.IsDebugBuild() ? LogLevel.Debug : LogLevel.Warn;
    ...
    private static void EnsureDevFlags()
    {
        ...
        try
        {
            ...
            var root = tree?.Root;
            _devFlags = root?.GetNodeOrNull("/root/DevFlags");
        }
        catch
        {
            _devFlags = null;
    ...
    {
        EnsureDevFlags();
        if (_devFlags == null) return false;
        try
        {
            var all = (bool)_devFlags.Get("debug_all");
            if (all) return true;
            // Kategorie-spezifisch
            return (bool)_devFlags.Get(cat);
        }
        catch
        {
            return false;
    ...
    {
        if (!ShouldLog(category, level, local)) return;
        ...
        System.Collections.Generic.Dictionary<string, object?>? data = null, string? correlationId = null, bool local = false)
    {
        ...
        if (level >= LogLevel.Warn) GD.PrintErr(line); else GD.Print(line);
    }
    ...
    private static string ValueToString(object? v)
    {
        ...
        if (v is StringName sn) return sn.ToString();
        if (v is Vector2I v2i) return $"({v2i.X},{v2i.Y})";
        if (v is Vector2 v2) return $"({v2.X:F1},{v2.Y:F1})";
